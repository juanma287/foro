...
Meteor.methods({
  postInsert: function(postAttributes) {
    ...
    // comprobamos que postAttributes pasado como argumento las cadenas title y descripcion, para no terminar insertando cualquier dato extraño en nuestra base de datos
    ...
    if (errors.title || errors.descripcion)
      throw new Meteor.Error('post-invalido', "Debe establecer un título y una descripción para su publicación");



    //  Si ya tenemos un post con el mismo titulo, no vamos a permitir que se añada una segunda vez, 
    ...
    // actualizamos el contador de post
    Temas.update(post.temaId, {$inc: {postCount: 1}});
